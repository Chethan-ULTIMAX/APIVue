...
function AuthVisual({ mode }: { mode: AuthMode }) {
  ...
  return (
    <div
      ...
      className={`auth-visual auth-visual-${mode} group relative min-h-[430px] overflow-hidden rounded-[28px] border p-7 sm:p-9 lg:min-h-[calc(100vh-48px)] lg:p-10`}
    >
      ...
      <div className="relative z-10 flex h-full min-h-[390px] flex-col justify-between lg:min-h-[calc(100vh-128px)]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-3 py-2 text-xs font-semibold backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.06] light:border-black/10 light:bg-black/[0.035]">
            <span className="grid h-6 w-6 place-items-center rounded-full bg-primary text-primary-foreground shadow-lg shadow-primary/30">
              <Sparkles className="h-3.5 w-3.5" />
            </span>
            APIVue intelligence
          </div>
          <span className="font-mono-id text-[10px] uppercase tracking-[0.22em] text-white/45 light:text-black/45">
            {mode === 'signin' ? 'return / connect' : 'new / identity'}
          </span>
        </div>
        ...
        <div className="relative mx-auto w-full max-w-[650px] flex-1 py-8 lg:py-12">
          <div className="auth-visual-scene absolute inset-0">
            ...
            <div className="auth-core-card" aria-hidden="true">
              ...
              <div className="relative rounded-2xl border border-white/10 bg-[#0c0e15]/90 p-4 shadow-2xl shadow-black/30 backdrop-blur-xl dark:border-white/10 dark:bg-[#0c0e15]/90 light:border-black/10 light:bg-white/90 light:shadow-black/10">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    <div className="text-[9px] font-bold uppercase tracking-[0.18em] text-white/40 light:text-black/40">Developer graph</div>
                    <div className="mt-1 text-sm font-semibold text-white light:text-slate-900">Everything in one view</div>
                  </div>
                  <div className="h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_12px_rgba(52,211,153,.8)]" />
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {['GitHub', 'Code', 'Security'].map((label, index) => (
                    <div key={label} className="rounded-xl border border-white/8 bg-white/[0.035] px-2 py-3 light:border-black/8 light:bg-black/[0.025]">
                      <div className="mb-2 h-1.5 w-1/2 rounded-full bg-primary/60" />
                      <div className="text-[9px] font-medium text-white/55 light:text-slate-500">{label}</div>
                      <div className="mt-2 h-1.5 w-full rounded-full bg-white/8 light:bg-black/8">
                        <div className="h-full rounded-full bg-gradient-to-r from-primary/70 to-cyan-400/70" style={{ width: `${58 + index * 13}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="max-w-xl">
          <div className="mb-4 flex items-center gap-2 text-xs font-semibold text-primary">
            ...
            {mode === 'signin' ? 'Pick up where you left off' : 'Build your developer identity'}
          </div>
          <h2 className="max-w-lg text-3xl font-semibold tracking-tight text-white sm:text-4xl light:text-slate-950">
            Connect the work you do. <span className="text-white/45 light:text-slate-500">See the story it creates.</span>
          </h2>
          <p className="mt-4 max-w-xl text-sm leading-6 text-white/55 light:text-slate-600">
            APIVue brings your developer tools into one calm, interactive space so your progress feels visible instead of scattered.
          </p>
          <div className="mt-6 flex flex-wrap gap-2">
            {['Progress', 'Profiles', 'Goals', 'Insights'].map((item) => (
              <span key={item} className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.045] px-3 py-1.5 text-[11px] font-medium text-white/65 backdrop-blur light:border-black/10 light:bg-black/[0.03] light:text-slate-600">
  ...
  return (
    <main className={`auth-page auth-page-${mode} min-h-screen overflow-hidden bg-background text-foreground`}>
      ...
      <div className="relative mx-auto grid min-h-screen max-w-[1540px] gap-0 p-3 sm:p-5 lg:grid-cols-2 lg:gap-5 lg:p-6">
        <section className={`auth-form-column auth-form-${mode} flex min-h-[720px] flex-col justify-between rounded-[28px] border border-border/70 bg-card/85 p-5 shadow-2xl shadow-black/5 backdrop-blur-xl sm:p-8 lg:min-h-[calc(100vh-48px)] lg:p-10`}>
          <header className="flex items-center justify-between">
            <Link to="/" className="group inline-flex items-center gap-2.5 rounded-full px-1 py-1.5 focus-ring" aria-label="APIVue home">
              <span className="grid h-9 w-9 place-items-center rounded-xl border border-border bg-background shadow-sm transition-transform duration-300 group-hover:rotate-3 group-hover:scale-105">
                <img src={logoSrc} alt="APIVue" className="h-6 w-6 object-contain" />
              </span>
              <span className="text-sm font-bold tracking-tight">APIVue</span>
            </Link>
            <button type="button" onClick={toggleTheme} className="focus-ring inline-flex h-9 w-9 items-center justify-center rounded-full border border-border bg-background/80 text-muted-foreground transition hover:-translate-y-0.5 hover:text-foreground" aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} theme`}>
          ...
          <div className="mx-auto flex w-full max-w-[440px] flex-1 flex-col justify-center py-10">
            <div className="mb-8">
              <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-primary/15 bg-primary/[0.06] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-primary">
        ...
        <section className="auth-visual-column hidden lg:block">
