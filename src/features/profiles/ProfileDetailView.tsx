...
import { GitHubContributionImage, TimeRangeChart } from '@/components/analytics/TimeRangeChart';
...
export function ProfileDetailView() {
  ...
  const metrics = useMemo(() => {
    ...
    const replacements: Record<string, number> = {
      followers: Number(liveMeta.followers ?? 0),
      following: Number(liveMeta.following ?? 0),
      public_gists: Number(liveMeta.public_gists ?? 0),
    };
  ...
  const refresh = async () => {
    setRefreshing(true);
    try { await sync.mutateAsync({ platform: profile.platform, handle: profile.handle }); await refetch(); toast({ title: 'Profile refreshed', description: 'Latest source data requested.' }); }
    catch (error) { toast({ title: 'Refresh failed', description: error instanceof Error ? error.message : 'Could not refresh profile.', variant: 'destructive' }); }
  ...
  return <div className="relative min-h-full overflow-hidden"><div className="pointer-events-none absolute -top-32 right-0 h-96 w-96 rounded-full bg-primary/[0.08] blur-3xl"/><div className="relative mx-auto max-w-7xl space-y-6 p-5 sm:p-6 lg:p-8">
    ...
    {profile.platform === 'github' && <section className="space-y-5"><GitHubContributionImage username={profile.handle} joinedAt={String(liveMeta?.created_at ?? profile.data?.joinedAt ?? '')}/><TimeRangeChart points={activity} tone={tone.chart} title="GitHub activity timeline" subtitle="Recent events returned by GitHub, with a readable 7-day / 30-day / 1-year / all-time view."/></section>}
    {profile.platform !== 'github' && <TimeRangeChart points={activity} tone={tone.chart} title={`${integration.name} activity`} subtitle="Real dated activity returned by the connected source."/>}
